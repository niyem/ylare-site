# Deployment Guide for `cuer.groupebm.net`
...
4. Under **Custom domain**, the field should already say `cuer.groupebm.net` because the `CNAME` file is in the repo. If not, type it in and click **Save**.
...
> **Note:** A CNAME record only works on a subdomain like `cuer.groupebm.net`, not the root domain. Since you wanted a subdomain, this is the right setup.

## Step 6, Wait, then verify

1. DNS usually propagates within 5 to 30 minutes (occasionally up to a few hours).
2. Test in a fresh browser tab: <http://cuer.groupebm.net>
...
- Try `nslookup cuer.groupebm.net` in PowerShell, it should return your GitHub Pages address.
